-- GriffithsRing.m2
--
-- Griffiths-Dwork pole-order reduction on the Grassmannian Gr(k, n).
-- Given a degree d hypersurface Z = V(f) in Gr(k, n), computes the graded
-- pieces (R / J_f)_{pd - n} of the Jacobian ring, which carry the primitive
-- Hodge numbers h^{N-p, p-1}_prim(Z) (where N = dim Z = k(n-k) - 1).
-- Results are printed alongside the expected Hodge numbers obtained from
-- Schubert2 via chi(Omega^j_Z) - chi(Omega^j_{Gr}).
--
-- Parameters (edit below):
--   k, n  -- Grassmannian Gr(k, n); requires 2 <= k <= n - 2
--   d     -- degree of f
--   K     -- coefficient field (QQ for exact, ZZ/p for speed)
--
-- f is sampled by randomIntPoly (every monomial of degree d with a random
-- integer coefficient). It is NOT verified smooth -- re-run if the
-- "Jacobian ideal does not have expected number of generators" check fires.
--
-- Run with:  M2 GriffithsRing.m2
-- Thesis reference: Chapter 2, Griffiths-Dwork reduction for Grassmannians.
...
for i from 1 to k*(n-k) list hilbertFunction((i)*d - n,J)
...
if primCohomGr(k,n) >0 then (
  if k*(n-k) % 2 == 0 then (
    -- contribution from H^N_prim(Gr(k,n)), when N even
      print("Contribution to (R/J_f)_{id-n} from H^N_van(Gr), with i = " | toString (1+k*(n-k)//2) | ": "| toString primCohomGr(k,n));
  ) else (
