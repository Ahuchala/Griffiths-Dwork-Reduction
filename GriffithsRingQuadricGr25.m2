-- GriffithsRingQuadricGr25.m2
--
-- Specialization of the GriffithsRing.m2 pipeline to a quadric Z = V(f)
-- in Gr(2, 5), with a fixed diagonal f = sum c_{ij} p_(i,j)^2. Computes
-- primitive Hodge numbers via R / J_f.
--
-- Run with:  M2 GriffithsRingQuadricGr25.m2
-- Companion smoothness check: SmoothCheckQuadricGr25.m2
-- Thesis reference: Chapter 4, worked example for the quadric in Gr(2, 5).
...
-- print basis with
for p from 3 to 4 list basis(p*d -n,J)